...
- **Internal linking of the tool series matters**: the 3 tool landing pages (`/deepseek-api-cost-tracker`, `/deepseek-cache-hit-rate-analyzer`, `/deepseek-api-pricing-calculator`) must stay linked from the homepage "Explore the Tool Series" section (`LandingPage.tsx`), the footer Tools row (`FooterBar.tsx`), and each other. Do not remove those links or the pages become orphaned.
